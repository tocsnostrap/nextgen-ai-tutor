...
async def list_generated_videos(
    ...
    import os
    video_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), "attached_assets", "generated_videos")
    if not os.path.exists(video_dir):
        return {"videos": []}
    files = [f for f in os.listdir(video_dir) if f.endswith(".mp4")]
    return {
        "videos": [
            {"filename": f, "url": f"/generated-videos/{f}"}
...
async def generate_educational_video(
    ...
    safe_topic = "".join(c if c.isalnum() or c in " -_" else "" for c in req.topic)[:60].strip().replace(" ", "_").lower()
    video_hash = hashlib.md5(f"{req.topic}_{req.subject}_{req.age}".encode()).hexdigest()[:10]
    filename = f"{safe_topic}_{video_hash}.mp4"
    video_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), "attached_assets", "generated_videos")
    ...
    if req.age <= 5:
        style_hint = "colorful, cartoonish, bright colors, simple shapes, friendly and fun"
    elif req.age <= 8:
        style_hint = "vibrant educational animation, clear visuals, engaging and kid-friendly"
    else:
        style_hint = "educational illustration, clean diagram style, informative and visually clear"

    user_style = f", {req.style}" if req.style else ""

    prompt = (
        f"Educational animated visualization about {req.topic} for {req.subject} class. "
        f"Show the concept visually with clear imagery. "
        f"{style_hint}{user_style}. "
        f"No text overlays. Smooth animation. High quality educational content."
    )
    ...
    return {
        ...
        "topic": req.topic,
        "prompt": prompt,
        "message": "Video generation has been requested. Pre-generated videos are available for common topics.",
    }
